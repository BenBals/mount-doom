...
std::optional<PartialSolution> ArticulationPointSolver::solve_applicable(FvsInstance &&instance) {
...
std::optional<PartialSolution> ArticulationPointSolver::solve(FvsInstance &&instance) {
  ...
  selected_component_without_node.forget_partial_solution();
  ...
  selected_component_with_node._upper_bound = solution_without->number_of_final_nodes() + 1;
  ...
  if (solution_with) {
    helpers::assert_and_log(solution_with.value().number_of_final_nodes() <=
                                solution_without->number_of_final_nodes(),
                            "sub solver gave too big solution");
    add_set_original_indices = std::set(solution_with->solution_nodes().begin(), solution_with->solution_nodes().end());
  } else {
    add_set_original_indices = std::set(solution_without->solution_nodes().begin(),
                                        solution_without->solution_nodes().end());
