...
std::optional<std::vector<size_t>> BranchingSolver::solve(FvsInstance &&instance) {
  ...
  for (auto &sub_instance : sub_instances) {
    ...
    if (result) {
      spdlog::debug("\t\t\tImproved bound from {} to {}", bound, result.value().size());
      bound = result.value().size();
      best = result;
    }
  }
  return best;
