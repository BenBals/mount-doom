...
MaxByKeyHeuristicSolver<F>::MaxByKeyHeuristicSolver(F &&keying_function,
    ...
    : NodeKeyed<F>(std::move(keying_function)), _reduce_after_taking_n(reduce_after_taking_n) {}
...
template <class F>
requires node_key_function<F> PartialSolution
MaxByKeyHeuristicSolver<F>::greedily_find_initial_solution(FvsInstance &&instance) {
  ...
  PartialSolution unfiltered_solution;
  size_t round = 0;
  ...
  while (current_cycle_opt) {
    ...
    if (round % _reduce_after_taking_n == 0) {
    ...
    for (auto new_node_original_index :
         instance._partial_solution.chosen_nodes_without_finalization()) {
    ...
    unfiltered_solution.merge(instance.forget_partial_solution());
  }
...
template <class F>
requires node_key_function<F>
    PartialSolution MaxByKeyHeuristicSolver<F>::make_solution_nodes_minimal(
        const FvsInstance &instance, const std::vector<size_t> &unfiltered_solution_initial) {
  PartialSolution filtered_solution;
  // map from original_index to node_index in initial reduced_instance
  ...
  for (auto original_index : unfiltered_solution_initial) {
  ...
  auto unfiltered_solution = unfiltered_solution_initial;
  while (!unfiltered_solution.empty()) {
    ...
    filtered_solution.add_node(current_original_index);
...
template <class F>
requires node_key_function<F> std::optional<PartialSolution>
MaxByKeyHeuristicSolver<F>::solve(FvsInstance &&instance) {
  ...
  PartialSolution final_solution = reduced_instance.forget_partial_solution();
  ...
  auto filtered_solution_minimal =
      make_solution_nodes_minimal(reduced_instance_copy, unfiltered_solution.solution_nodes());
  auto filtered_solution = iterative_small_compression(reduced_instance_copy,
                                                       filtered_solution_minimal.solution_nodes());

  final_solution.merge(std::move(filtered_solution));

  if (final_solution.number_of_final_nodes() < initial_upper_bound)
...
template <class F>
requires node_key_function<F>
    PartialSolution MaxByKeyHeuristicSolver<F>::iterative_small_compression(
        const FvsInstance &instance, const std::vector<size_t> &unfiltered_solution) {
  ...
  while (found_improvement) {
    ...
    for (size_t cycle_idx = 0; cycle_idx < filtered_solution.size(); cycle_idx++) {
      time = 2 * cycle_idx + 1;
    ...
    for (size_t first_excluded_idx = 0;
         first_excluded_idx < filtered_solution.size() && !found_improvement;
         first_excluded_idx++) {
  ...
  PartialSolution filtered_solution_ps;
  for (auto idx : filtered_solution) {
    filtered_solution_ps.add_node(instance._original_indices[idx]);
  }
  return filtered_solution_ps;
}
