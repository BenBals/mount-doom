...
size_t FvsInstance::partial_solution_size() const {
  return _partial_solution.number_of_final_nodes();
}

PartialSolution FvsInstance::forget_partial_solution() {
  helpers::assert_and_log(_upper_bound > _partial_solution.number_of_final_nodes(),
                          "Partial solution was not smaller than upper bound and forced to forget");
  _upper_bound -= _partial_solution.number_of_final_nodes();
  auto result = std::move(_partial_solution);
...
split_fvs_instance_by_sccs(FvsInstance &&instance) {
  ...
  if (num_colors <= 1) {
    auto partial_solution = instance.forget_partial_solution();
