...
public:
  ...
  // indices exactly as given in the input files, NB they start at 1
  std::vector<size_t> _partial_solution;
  ...
  void collect_self_loops_into_partial_fvs();
  ...
  std::set<size_t> partial_solution_set() const;
  ...
  friend std::pair<std::vector<FvsInstance>, std::vector<size_t>>
