...
bool Graph::Node::operator==(const Node &other) const {
  return _idx == other._idx && _in.size() == other._in.size() && _out.size() == other._out.size() &&
         _undirected.size() == other._undirected.size() &&
...
void Graph::add_arc_unsafe(const size_t from, const size_t to) {
  ...
  if (from == to) {
    // self loop has to be put into undirected edges directly
...
std::optional<std::vector<size_t>>
Graph::find_limited_cycle_with_node(size_t start_index, Graph::CycleFindingData &data,
