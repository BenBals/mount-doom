...
std::optional<std::chrono::time_point<std::chrono::high_resolution_clock, std::chrono::nanoseconds>>
    runout_time_point;
...
void set_timeout_inner(time_t seconds, long nano_seconds) {
  runout_time_point = {std::chrono::high_resolution_clock::now() + std::chrono::seconds(seconds) +
...
set_timeout_to(std::chrono::time_point<std::chrono::high_resolution_clock> time_point) {
  auto nano_duration = time_point - std::chrono::high_resolution_clock::now();
  nano_duration = std::max(nano_duration, decltype(nano_duration)::zero()); // prevent underflow
...
std::optional<std::chrono::nanoseconds> remaining_time() {
  if (!runout_time_point)
    return {};
  return std::chrono::duration_cast<std::chrono::nanoseconds>(
      runout_time_point.value() - std::chrono::high_resolution_clock::now());
}
