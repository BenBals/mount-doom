...
TEST(GraphOperations, FindDisjointCyclesResultsAreDisjoint) {
  for (size_t i = 1; i <= 10000; i++) {
    auto num_nodes = 2 + gen() % 20ul;
    auto num_arcs = gen() % (num_nodes * num_nodes / 2);
    spdlog::info("Checking find cycles gives *disjoint* cycles with n={}, m={}, seed={}", num_nodes, num_arcs,
                 i);
    ...
    ASSERT_EQ(std::unique(all_nodes.begin(), all_nodes.end()), all_nodes.end()) << "Cycles were not disjoint";
  }
}
TEST(GraphOperations, FindDisjointCyclesComplementIsAcyclic) {
  for (size_t i = 1; i <= 10000; i++) {
    auto num_nodes = 2 + gen() % 20ul;
    auto num_arcs = gen() % (num_nodes * num_nodes / 2);
    spdlog::info("Checking find cycles complement is acyclic with n={}, m={}, seed={}", num_nodes, num_arcs,
                 i);
