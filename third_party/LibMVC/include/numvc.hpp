...
public:
  ...
  NuMVC(
      ...
      unsigned int rnd_seed = std::chrono::high_resolution_clock::now().time_since_epoch().count())
      : verbose(verbose), cutoff_time(std::chrono::duration_cast<duration_ms>(cutoff_time)),
...
private:
  ...
  inline int degree(int v) const { return v_beg_idx[v + 1] - v_beg_idx[v]; }
  ...
  template <typename Is> void build_instance(Is &str) {
    char line[1024];
    char tempstr1[10];
    char tempstr2[10];
    int e;
  ...
  void build_instance(const int &num_vertices, const std::vector<std::pair<int, int>> &edges) {
    v_num = num_vertices;
    e_num = edges.size();
  ...
  void init_sol() {
    ...
    if (verbose) {
      ...
      std::cout << "Initial solution time: " << init_sol_time.count() << "ms" << std::endl;
    }
...
public:
   ...
   */
  template <typename Duration> void set_cutoff_time(Duration d) {
  ...
  inline std::chrono::milliseconds get_best_duration() const {
    return std::chrono::duration_cast<std::chrono::milliseconds>(best_comp_time);
  }
   ...
   */
  static bool default_stats_printer(const NuMVC &solver, bool better_cover_found) {
    if (better_cover_found) {
      auto time_ms =
          std::chrono::duration_cast<std::chrono::milliseconds>(solver.get_best_duration());
      spdlog::info("NuMVC: Better VC found\tSize: {}\tTime: {:>5}ms\tRemaining: {:>5}ms",
                   solver.best_c_size, time_ms.count(), solver.cutoff_time.count() - time_ms.count());
    }
    return false;
  }
};
