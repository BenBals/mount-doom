...
std::vector<vertex_type> numvc_local_search(const Graph &graph) {
  using namespace std::chrono;
  using namespace libmvc;

  auto edge_list = graph.get_edge_list();
  std::cerr << "NuMVC " << graph.size() << std::endl;

  // TODO: run lower bounds first and set optimal size as lowerbound
  auto numvc = NuMVC(edge_list, graph.num_of_vertices(), 0, milliseconds(10*graph.size()), false);
  numvc.cover_LS();
  return numvc.get_cover(false); // false makes 0-based
}
