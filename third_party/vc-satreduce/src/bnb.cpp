...
Solution Solver::get_upper_bound() {
    ...
    if (!graph_.empty_graph()) {
        if (flags.ub == ub_algorithm::greedy) {
            ...
            project_solution(s, vc);
            return s;
        } else if (flags.ub == ub_algorithm::numvc) {
          auto start = std::chrono::high_resolution_clock::now();
          auto vc = graph_.num_of_vertices() > 50 ? algorithms::numvc_local_search(graph_)
                                                  : algorithms::iterated_local_search(graph_);
          auto end = std::chrono::high_resolution_clock::now();
          stats_.upper_bound_time += end - start;
