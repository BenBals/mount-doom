...
void branch_and_reduce_algorithm::addStartingSolution(std::vector<int> solution, int solutionSize) {
    if(solution.size() != y.size()) {
        cout << "ERROR: invalid solution vector!" << endl << flush;
    }
...
void branch_and_reduce_algorithm::force_into_independent_set(vector<NodeID> const &nodes) {
    ...
    for (NodeID const node : nodes) {
        ...
        if (x[node] != -1) {
            cerr << "ERROR: invalid vertex selected for independent set! 1" << endl << flush;
        }
...
////    cout << "Extending finer independent set" << endl << flush;
    ...
    for (size_t index = 0; index < independent_set.size(); ++index) if (independent_set[index]) {
        ...
        if (x[index] != -1) {
            cerr << "ERROR: invalid vertex selected for independent set! 2" << endl << flush;
        }
